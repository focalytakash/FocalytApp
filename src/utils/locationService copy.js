...
// Initialize Google Geocoding with provided API key
const GOOGLE_MAPS_API_KEY = '';
Geocoder.init(GOOGLE_MAPS_API_KEY);
console.log('🗺️ Google Places API initialized with key');
...
export const getAddressFromCoordinates = async (latitude, longitude) => {
  try {
    console.log('🗺️ Getting address for coordinates (Google only):', latitude.toFixed(6), longitude.toFixed(6));
    // Only use Google Places API
    const address = await getGoogleAddress(latitude, longitude);
    if (address && !address.error) {
      return address;
    }
    // Fallback address
    return createFallbackAddress('Could not determine address from Google API');
  } catch (error) {
    console.error('❌ Google geocoding error:', error);
    return createFallbackAddress(error.message || 'Google geocoding failed');
  }
};
...
const getGoogleAddressDirect = async (latitude, longitude) => {
  try {
    console.log('🌟 Using Google Geocoding Direct API...');
...
const getFallbackLocation = (includeAddress = true, errorMessage = 'No location available') => {
  const fallback = {
    latitude: 28.7041,  // Delhi coordinates for testing
      longitude: 77.1025,
    accuracy: 5000,
    timestamp: Date.now(),
    error: errorMessage
    ...
    if (testLocation && testLocation.latitude && testLocation.longitude) {
      ...
      if (hasPermission) {
        Alert.alert(
          ...
          'App ko permission mil gayi hai, lekin device se location fetch nahi ho rahi. Kripya location service ON karein ya open sky me try karein.'
        );
      }
      return false;
    }
